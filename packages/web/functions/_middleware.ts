...
function checkAccess(request: Request, env: Env): { allowed: boolean; redirect?: Response } {
...
async function verifyJWT(token: string, env: Env): Promise<Record<string, unknown> | null> {
  ...
  const valid = await crypto.subtle.verify('HMAC', key, sig as BufferSource, data);
