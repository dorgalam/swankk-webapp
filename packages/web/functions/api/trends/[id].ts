...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  const allowed = ['name', 'slug', 'context', 'designer_slugs', 'preview_images', 'images'];
  ...
  values.push(id);
  await context.env.DB.prepare(`UPDATE trends SET ${fields.join(', ')} WHERE id = ?`).bind(...values).run();

  const row = await context.env.DB.prepare('SELECT * FROM trends WHERE id = ?').bind(id).first();
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  const id = context.params.id;
  await context.env.DB.prepare('DELETE FROM trends WHERE id = ?').bind(id).run();
