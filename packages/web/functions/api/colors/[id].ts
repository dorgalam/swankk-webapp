...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  const allowed = ['name', 'slug', 'hex', 'description', 'main_image_url', 'images', 'products', 'related_tags'];
  ...
  values.push(id);
  await context.env.DB.prepare(`UPDATE colors SET ${fields.join(', ')} WHERE id = ?`).bind(...values).run();

  const row = await context.env.DB.prepare('SELECT * FROM colors WHERE id = ?').bind(id).first();
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  const id = context.params.id;
  await context.env.DB.prepare('DELETE FROM colors WHERE id = ?').bind(id).run();
