...
export default function Saved() {
  ...
  const items = bookmarks.get(activeTab);

  const handleRemove = (id: string) => {
    bookmarks.remove(activeTab, id);
    refresh();
  };
  ...
  const handleItemClick = (item: any) => {
    ...
    } else if (activeTab === "keywords" && item.slug) {
      if (item.type === "trend") {
    ...
    <div className="min-h-[60vh] pb-20">
      <div className="px-5 md:px-8 pt-8 pb-4">
        ...
        <div className="flex gap-1 border-b border-gray-100">
          {TABS.map(({ key, label }) => {
            const count = bookmarks.get(key).length;
            return (
              <button
                key={key}
      ...
      <div className="px-5 md:px-8">
        {items.length === 0 ? (
          ...
          <div
            className={
              activeTab === "designers" || activeTab === "keywords"
