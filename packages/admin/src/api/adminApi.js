...
function parseDesignerRow(row) {
  if (!row) return null
  return {
    ...row,
    known_for_tags: JSON.parse(row.known_for_tags || '[]'),
    eras: JSON.parse(row.eras || '[]'),
    signature_pieces: JSON.parse(row.signature_pieces || '[]'),
  }
}
...
const adminApi = {
  ...
  designers: {
    async list() {
      ...
      return results.map(parseDesignerRow)
    },
    async getById(id) {
      const { results } = await query(
      ...
      return parseDesignerRow(results[0])
    },
    async create(data) {
